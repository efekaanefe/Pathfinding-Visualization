...
def bfs(start, target):
    ...
    while not queue.empty():
        ...
        if current == target:
            return path, visited  # Return the path and visited cells
...
def uniform_cost_search(start, target):
    ...
    while not priority_queue.empty():
        ...
        if current == target:
            return path, visited.keys()  # Return the path and visited cells
    ...
    return [], visited.keys()  # Return an empty path if no path is found
...
def start_search():
    if algorithm == "Breadth-First":
        path, visited_cells = bfs(start, target)
    elif algorithm == "Depth-First":
        path, visited_cells = dfs(start, target)
    elif algorithm == "Uniform-Cost":
        path, visited_cells = uniform_cost_search(start, target)
    elif algorithm == "Greedy":
        path, visited_cells = greedy_search(start, target)
    elif algorithm == "A*":
        path, visited_cells = a_star(start, target)
    
    # render_grid(path=path, visited=visited_cells)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        # Handle mouse clicks for setting start, target, or selecting an algorithm
            ...
            if y < GRID_HEIGHT:
                if not start:
                    ...
                    # Clear start and target to allow re-selection
                    start, target = (y, x), None
                path = []; visited_cells = []

            else:
                # Algorithm selection from buttons
                for i, algo in enumerate(SEARCH_ALGORITHMS):
                    if 10 <= event.pos[0] <= 160 and HEIGHT - BUTTONS_START_MARGIN + i * 30 <= event.pos[1] <= HEIGHT - BUTTONS_START_MARGIN + i * 30 + 25:
                        algorithm = algo
        
        # Start search when 'S' key is pressed
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_s:
            if start and target:
                path, visited_cells = start_search()
